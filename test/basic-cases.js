...
function getRendererOutput(renderer, commands) {
  ...
  commands.forEach(c => {
    ...
    const events = makeArray(
      renderer.render(command(pressed, id, velocity, channel))
    );
    out.push(events);
  })
  return out;
...
MidifilePerformer.onRuntimeInitialized = function() {
  const renderer = new MidifilePerformer.Performer({
    unmeet:             true,
    complete:           false,
  ...
  test('chord velocities', function(t) {
    ...
    const model = renderer.getChronology();
    
    output = getRendererOutput(renderer, makeChordCommand(127));
    // console.log(output);

    renderer.setChronology(model);
  ...
  test('ill-formed score', function(t) {
    t.plan(1);
    const score = [
      [ 0,  false,  60 ], // end events before first start event should be discarded
      [ 10, true,   61, 127 ],
      [ 10, true,   62, 127 ],
      [ 10, false,  62 ],
      [ 10, false,  61 ],
    ...
    let output;
    writeScoreToRenderer(renderer, score);
    output = getRendererOutput(renderer, commands);
    console.log(output);
    t.equal(true, true);
  }); 
};
